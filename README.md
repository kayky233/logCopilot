...
| 👥 **多用户隔离 + 手册共享** | 日志/配置独立隔离，故障手册领域内全局共享 |
...
> 📖 **详细使用教程请参考 [用户手册](doc/USER_GUIDE.md)**
...
| 资源 | 路径 |
|------|------|
| 多组件实现与验证方案 | `doc/architecture_implementation_validation_plan.md` |
| 可运行诊断内核 | `backend/services/diagnosis_core.py` |
| 低成本验证入口 | `backend/services/validation_service.py` |
| 核心测试 | `tests/test_multicomponent_core.py` |
